...
#define CSIZE 65536   /* Concenteration Size */
#define DEGREE 8    /* degree - maximum number of prefteches */
#define IT_SIZE 256   /* size of index table */
#define GHB_SIZE 256  /* Number of entries in GHB */


// Global Histroy Table entry 
struct GHB_entry{
  md_addr_t miss_address; 
  struct GHB_entry * next;
  struct GHB_entry * prev;
  struct GHB_entry * CZone_next;
  struct GHB_entry * CZone_prev;
};
...
struct cache_t
{
  ...
  // head-tail of ghb in doubly linked list. Used to avoid dealing with NULL corner cases
  struct GHB_entry * ghb;
  struct GHB_entry * ghb_tail;
  int ghb_size;
  // Index Table
  struct IT_entry * it; 
  /* ECE552 Assignment 4 - END CODE */
...
//Stride Prefetcher helper function
